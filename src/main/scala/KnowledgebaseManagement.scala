...
object KnowledgebaseManagement extends DialogSession {
  ...
  def createKnowledgeBase(projectId: String, displayName: String): Either[Throwable, KnowledgeBase] = {
    try {
      val knowledgeBasesClient = KnowledgeBasesClient.create()
  ...
  def getKnowledgeBase(projectId: String, knowledgeBaseId: String): Either[Throwable, KnowledgeBase] = {
