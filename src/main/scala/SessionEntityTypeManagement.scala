...
import com.google.cloud.dialogflow.v2.{SessionEntityType, SessionEntityTypeName, SessionEntityTypesClient, SessionName}

import scala.collection.JavaConverters._

object SessionEntityTypeManagement extends DialogSession {

  def listSessionEntityTypes(projectId: String, sessionId: String): Either[Throwable, Iterable[SessionEntityType]] = {
    try {
  ...
  def createSessionEntityType(projectId: String, sessionId: String,
                              entityValues: List[String], entityTypeDisplayName: String, entityOverrideMode: Int): Either[Throwable, SessionEntityType]
  = {
    try {
      val sessionEntityTypesClient = SessionEntityTypesClient.create()
      val session = SessionName.of(projectId, sessionId)
      ...
      val sessionEntityType = SessionEntityType.newBuilder()
  ...
  def deleteSessionEntityType(projectId: String, sessionId: String,
